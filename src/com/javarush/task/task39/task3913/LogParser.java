...
public class LogParser implements IPQuery, UserQuery, DateQuery, EventQuery, QLQuery {
    ...
    public Set<Date> getDatesForUserAndEvent(String user, Event event, Date after, Date before){
        ...
        for (int i = 0; i < logEntities.size(); i++) {
            if (dateBetweenDates(logEntities.get(i).getDate(), after, before)) {
                if (logEntities.get(i).getEvent().equals(event) && logEntities.get(i).getUser().equals(user)) {
    ...
    @Override
    public Set<Date> getDatesWhenUserDownloadedPlugin(String user, java.util.Date after, java.util.Date before){
        Set<Date> result = new HashSet<>();
        for (int i = 0; i < logEntities.size(); i++) {
            if (dateBetweenDates(logEntities.get(i).getDate(), after, before)) {
    ...
    public Set<String> getIPsForUser(String user, Date after, Date before){
        ...
        for (int i = 0; i < logEntities.size(); i++) {
            if(dateBetweenDates(logEntities.get(i).getDate(), after, before)) {
                if(logEntities.get(i).getIp().equals(user)) {
                    result.add(logEntities.get(i).getIp());
                }
            }
        }
    ...
    public Set<Object> execute(String query) {
        ...
        if (matcher.find()) {
            ...
            String afterStr = matcher.group(4);
            String beforeStr = matcher.group(5);

            Date after = null;
            Date before = null;
            try {
                if (afterStr != null && beforeStr != null) {
                    after = simpleDateFormat.parse(afterStr);
                    before = simpleDateFormat.parse(beforeStr);
                }
            ...
            for (LogEntity entity : logEntities) {
                ...
                if (filterField != null && filterValue != null) {
                    Object value = getCurrentValue(entity, filterField);
                    if ("date".equals(filterField)) {
                        try {
                            Date filterDate = simpleDateFormat.parse(filterValue);
                            matches = filterDate.getTime() == ((Date) value).getTime();
                        } catch (ParseException e) {
                            matches = false;
                        }
                    } else {
                        matches = filterValue.equals(value.toString());
                    }
                }
                if (matches && after != null && before != null) {
                    Date entityDate = entity.getDate();
                    if (entityDate.before(after) || entityDate.after(before)) {
                        matches = false;
                    }
                }
                if (matches) {
                    result.add(getCurrentValue(entity, field));
                }
            }
        }
        return result;
    ...
    private Object getCurrentValue(LogEntity logEntity, String field) {
        ...
        switch (field) {
            case "ip":
                result = logEntity.getIp();
                break;
            case "user":
                result = logEntity.getUser();
                break;
            case "date":
                result = logEntity.getDate();
                break;
            case "event":
                result = logEntity.getEvent();
                break;
            case "status":
                result = logEntity.getStatus();
                break;
        }
        return result;
    }
